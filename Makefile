...
.PHONY: 
check-lint:
	find src -name '*pp' -type f | xargs $(CLANG_TIDY) --quiet --warnings-as-errors='*' --extra-arg=-std=c++20
	find src -name '*pp' -type f | xargs $(CLANG_FORMAT) --Werror --dry-run

# Linters
.PHONY: 
lint:
	find src -name '*pp' -type f | xargs $(CLANG_TIDY) --quiet --extra-arg=-std=c++20 --fix
